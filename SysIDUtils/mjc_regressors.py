...
def calculate_regressor (mj_model,mj_data, q, dq, ddq) -> npt.ArrayLike:
    """Calculate the regressor matrix at a given configuration, velocity, and acceleration.
    ...
    Returns:
        npt.ArrayLike: Regressor matrix
    """
    n_samples = q.shape[0]
    no_params = mj_model.njnt * 10
    W = np.empty((0, no_params))
    # reset the state
    mujoco.mj_resetData(mj_model, mj_data)
    
    for i in range(n_samples):
        # set state to mj_data
        mj_data.qpos[:] = q[i, :]
        mj_data.qvel[:] = dq[i, :]
        mj_data.qacc[:] = ddq[i, :]
        mujoco.mj_forward(mj_model, mj_data) # we need to perform forward kinematics to update the data
        W_sample = joint_torque_regressor(mj_model, mj_data)
        W = np.vstack((W, W_sample))
    return W
